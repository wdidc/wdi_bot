...
module.exports = function(){
  ...
  h.numsBetween = function(min, max, nums){
    var set = [], n;
    console.log(nums);
    for(r = 0; r < nums.length; r++){
      n = Number.parseInt(nums[r]);
      if(!isNaN(n) && n >= min && n <= max) set.push(Math.round(n));
    }
    return set;
  }

  h.averages = function(nums){
    var quantities = h.mode(nums), variance, output = {};
    output.data         = {};
    output.data["Mean"] = h.mean(nums);
    output.data["Mode"] = quantities[1];
    output.data["Median"] = h.median(nums);
    output.data["Variance"] = h.variance(nums, output.data["Mean"]);
    output.data["Standard Deviation"] = h.standardDeviation(output.data["Variance"]);
